...
def render_flow(entities: EntitySet, question_targets: dict[tuple[str, str], str]) -> str:
    ...
    return (
        ...
        "<div class='view-heading'><h2 id='flow-heading'>Survey flow</h2></div>"
        ...
        "<div class='flow-map-controls' hidden><button id='flow-expand' type='button'>Expand map</button>"
        "<button id='flow-collapse' type='button'>Collapse map</button></div></div>"
        ...
        + "".join(maps)
        + "</div><aside class='flow-walkthrough' aria-labelledby='flow-walkthrough-heading'>"
        "<h3 id='flow-walkthrough-heading'>Try a scenario</h3>"
        ...
        "<div id='flow-interactive' hidden><label for='flow-survey-select'>Walkthrough survey</label>"
        "<select id='flow-survey-select'></select><p id='flow-walkthrough-status' role='status' aria-live='polite'></p>"
        ...
        "<ol id='flow-assumption-list'></ol></details></div></aside></div></section>"
