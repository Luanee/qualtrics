...
def report(
    ...
    manifest: Annotated[
        Path | None, typer.Option(exists=True, help="Survey metadata manifest for explicit files.")
    ] = None,
    surveys: Annotated[Path | None, typer.Option(exists=True)] = None,
    sections: Annotated[Path | None, typer.Option(exists=True)] = None,
    ...
    if len(entity_folders) > 1 and (paths or manifest):
        raise typer.BadParameter("explicit entity paths cannot be combined with multiple folders")
    load_paths = {**paths, **({"manifest": manifest} if manifest is not None else {})}
    entity_sets = [load_entities(entity_folder, **load_paths) for entity_folder in entity_folders]
    entities = (
        merge_entity_sets(entity_sets)
        if len(entity_sets) > 1
        else entity_sets[0]
        if entity_sets
        else load_entities(**load_paths)
    )
    try:
