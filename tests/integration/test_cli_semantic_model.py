...
def test_semantic_model_cli_writes_ten_json_tables(tmp_path: Path, survey_files: tuple[Path, Path]) -> None:
    ...
    assert {path.name for path in output.glob("*.json")} == {
        ...
        "fact_comments.json",
        "fact_comment_translations.json",
        "dim_surveys.json",
        "dim_questions.json",
        "dim_answer_options.json",
        "dim_display_languages.json",
        "dim_question_labels.json",
        "dim_answer_option_labels.json",
        "manifest.json",
    }
