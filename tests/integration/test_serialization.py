...
def test_wide_survey_metadata_round_trips_csv_and_supports_semantic_model(tmp_path: Path) -> None:
    entities = _wide_survey(tmp_path)
    assert len(entities.surveys[0]["source_columns_json"]) > 131_072
    ...
    write_entities(entities, tmp_path / "json", "json")

    json_loaded = load_entities(tmp_path / "json")
    assert json_loaded.surveys[0]["source_columns_json"] == entities.surveys[0]["source_columns_json"]
    for loaded in (
        load_entities(tmp_path / "csv"),
        load_entities(**{name: tmp_path / "csv" / f"{name}.csv" for name in ENTITY_NAMES}),
    ):
        assert loaded.surveys[0]["source_columns_json"] == entities.surveys[0]["source_columns_json"]
        assert json.loads(loaded.surveys[0]["source_columns_json"]) == json.loads(
            entities.surveys[0]["source_columns_json"]
        )
...
def test_csv_reader_limit_handles_platform_overflow_without_lowering_existing_limit(
    ...
    entities = _wide_survey(tmp_path)
    manifest = entities.surveys[0]["source_columns_json"]
    assert 131_072 < len(manifest) < 250_000
    write_entities(entities, tmp_path / "csv", "csv")
    ...
    try:
        ...
        assert real_field_size_limit() >= max(initial_limit, len(manifest))
    finally:
