...
Reparse repeated single-choice exports to recover options previously omitted after the first field. For example, `1_QID1` and `2_QID1` each need the full choice list, even if one iteration has no answers. This adds field-specific options and resolves matching answers in later iterations. Existing occurrence IDs, existing option IDs, and raw values stay unchanged; affected catalog IDs can change because their normalized content includes option domains. Rebuild combined entities, reports, and semantic tables from the reparsed data so their catalog references agree.
