...
```text
uv run --extra cli --extra ui qualtrics entities combine data/survey-a/entities data/survey-b/entities --output data/combined/entities --format parquet
```

Parquet is the default for this command and works with the base package. To produce tables for a spreadsheet, use a separate output path:
...
```text
uv run --extra cli --extra ui qualtrics report --folder data/combined/entities --output data/combined/report.html
```
...
The output folder has one `manifest.json` keyed by survey ID. It retains each survey's own flow and source-column descriptors, with storage-column names updated when combining resolves a collision. Use that survey's entry to trace a combined response property back to its original export column.
...
```text
uv run --extra cli --extra ui qualtrics entities combine data/survey-a data/survey-b --output data/combined-v2/entities --format parquet
```
...
```text
uv run --extra cli --extra ui qualtrics entities combine data/api-export --output data/api-combined/entities --format parquet
```
