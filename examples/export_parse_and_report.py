"""Export one Qualtrics survey, extract its CSV, parse it, and render a report."""
...
def main(
    survey_id: Annotated[str, typer.Argument(help="Qualtrics survey ID, for example SV_abc123")],
    ...
    """Export a survey and create its CSV, entities, and HTML report."""
    if Path(survey_id).name != survey_id or survey_id in {".", ".."}:
        raise typer.BadParameter("survey_id must not contain path separators")
