...
- `serialization/tables.py` shares JSON, CSV, and Parquet file writing. Entity and semantic adapters retain their own ordered columns, scalar types, and normalization policies; similarly named columns need not have identical types. SQLite retains its separate atomic, no-overwrite writer.
